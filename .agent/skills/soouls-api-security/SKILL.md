---
name: soouls-api-security
description: The Soouls API namespace pattern. Use when adding, modifying, or debugging tRPC routes in packages/api, or when working with the NestJS backend in apps/backend. Covers the constants.ts + run.ts split, Zod validation, rate limiting, auth middleware, and the full request lifecycle. Required reading before touching any file in packages/api/src/namespaces/.
---

# Soouls API Security & Namespace Pattern
