...
export default function CommandCenterSignUpPage() {
  const [showClerk, setShowClerk] = useState(true);
    ...
    <main className="relative flex min-h-screen flex-col items-center justify-center overflow-hidden bg-[#040814] px-6 py-10 selection:bg-amber-500/30">
      ...
      <div className="relative z-10 flex max-w-2xl flex-col items-center text-center">
        ...
        <p className="mt-8 max-w-lg text-base leading-relaxed text-slate-500">
          If you don&apos;t have an invite, stop here. New accounts are not open-registration.
        </p>

        <div className="mt-10 flex flex-wrap items-center justify-center gap-3">
          <FeaturePill icon={<Shield className="h-3.5 w-3.5" />} label="RBAC Enforcement" />
          <FeaturePill icon={<Sparkles className="h-3.5 w-3.5" />} label="Audit-Logged Access" />
          <FeaturePill icon={<Fingerprint className="h-3.5 w-3.5" />} label="Invite-only" />
        </div>
