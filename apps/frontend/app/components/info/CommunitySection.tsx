        ...
        <div className="flex flex-col items-center text-center">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 w-full">
            {[
              ...
              >
                <div className="p-4 rounded-2xl bg-white/5 group-hover:scale-110 transition-transform" style={{ color: item.color }}>
                   {item.icon}
                </div>
                <div>
                   <h3 className="font-urbanist text-2xl font-bold text-[#EFEBDD] mb-2">{item.label}</h3>
                   <p className="font-urbanist text-[#A8A8A8] leading-relaxed">{item.text}</p>
                </div>
                <span className="font-urbanist text-xs font-bold text-[#D6C2A3] uppercase tracking-widest mt-auto group-hover:text-[#E07A5F]">Join sanctuary →</span>
              </a>
            ))}
          </div>
        </div>
