...
/**
 * Manages client-only persisted entry state (text and blocks) synchronized with localStorage.
 *
 * Provides hydration status, debounced localStorage persistence with a save-status indicator, setter wrappers that auto-persist, and a clear-all helper.
 *
 * @returns An object with:
 *  - `textContent` — current entry text
 *  - `setTextContent` — updater for `textContent` that persists changes
 *  - `blocks` — current array of blocks
 *  - `setBlocks` — updater for `blocks` that persists changes
 *  - `hydrated` — `true` after state is loaded from localStorage
 *  - `saveStatus` — `'idle' | 'saving' | 'saved'` reflecting local persist state
 *  - `clearAll` — removes persisted data and resets in-memory state
 */
...
// MODAL PRIMITIVES
/**
 * Renders a fullscreen modal backdrop that centers its children.
 *
 * @param children - The content to display centered inside the overlay
 * @returns A fullscreen, centered overlay element with a translucent dark backdrop and backdrop blur
 */
...
/**
 * Renders a modal panel with a header (icon + title), optional extra controls, body content, and a footer with actions and a close button.
 *
 * @param title - Header title text shown next to the icon
 * @param icon - Node rendered before the title in the header
 * @param onClose - Callback invoked when the modal's close button is clicked
 * @param extra - Optional node rendered alongside the close button (e.g., secondary controls)
 * @param footer - Node rendered in the modal footer (action buttons)
...
// MODALS
/**
 * Provides a modal drawing canvas for creating and editing a doodle.
 *
 * @param onClose - Called when the modal is dismissed without saving.
 * @param onSave - Called with the doodle image as a data URL (PNG) when the user adds the doodle to the entry.
 * @returns The rendered doodle modal React element.
 */
...
/**
 * Modal UI that lets the user pick or drop an image, preview it, and add it to the current entry.
 *
 * @param onClose - Called to close the modal without adding an image
 * @param onAdd - Called when the user confirms; receives the image data URL and the original filename
 * @returns The modal element containing the image dropzone/preview and Add/Cancel controls
 */
...
/**
 * Renders a modal for creating a goal with an optional label and starting its timer.
 *
 * @param onClose - Called when the modal is dismissed.
 * @param onAdd - Called with the entered `goal` and `label` when the user confirms starting the timer.
 * @returns The Goal modal JSX element.
 */
...
/**
 * Renders a modal for creating a task list with a title and multiple tasks.
 *
 * @param onClose - Callback invoked to close the modal.
 * @param onAdd - Callback invoked when the user confirms the list; receives the list `title` and an array of task strings (only tasks with non-empty trimmed text).
 * @returns The rendered Task List modal element.
 */
...
/**
 * Visual container for content blocks that includes a floating remove control.
 *
 * Renders a styled rounded panel wrapping the provided children and displays an absolute-positioned remove button that calls `onRemove` when clicked.
 *
 * @param children - Elements to render inside the card
 ...
 * @returns The card element containing `children` and the remove control
 */
...
/**
 * Render a card displaying an image block with a remove control.
 *
 * Displays the image thumbnail and a small badge; invoking the card's remove control calls `onRemove`.
 *
 * @param b - The image block to render (contains `dataUrl` and `name`)
 * @param onRemove - Callback invoked when the card's remove action is triggered
 * @returns The JSX element for the image block card
 */
...
/**
 * Renders a voice-note card with play/pause control, animated progress visualizer, duration display, and a remove action.
 *
 * @param b - The voice block containing `dataUrl` and `duration` used for playback and display.
 * @param onRemove - Callback invoked when the card's remove action is triggered.
 * @returns The React element for the voice note card.
 */
...
/**
 * Renders a doodle block as a card showing the doodle image, a "Doodle added" badge, and a remove control.
 *
 * @param b - The doodle block whose `dataUrl` is displayed as the image
 * @param onRemove - Callback invoked when the user requests removal of this block
 * @returns The rendered JSX element for the doodle card
 */
...
/**
 * Renders a goal timer card showing the goal text, the formatted elapsed time, the current clock time, and controls to start/pause or reset the timer.
 *
 * @param b - The goal block to render; expected fields include `goal` (text), `label` (optional badge text), `seconds` (elapsed seconds), and `running` (whether the timer is active).
 * @param onUpdate - Called with an updated block when the timer state, elapsed seconds, or other block fields change.
 * @param onRemove - Called when the card's remove control is activated.
 * @returns The rendered GoalCard React element.
 */
...
/**
 * Render a tasklist block as an interactive card with toggleable tasks and a progress badge.
 *
 * @param b - The `tasklist` block containing the card title and an array of tasks.
 * @param onUpdate - Called with the updated block when a task's `done` state changes.
 * @param onRemove - Called when the card's remove action is triggered.
 * @returns The rendered tasklist card element.
 */
...
/**
 * Render a styled toolbar button with an icon, label, optional count badge, and active state styling.
 *
 * @param icon - Icon node shown to the left of the label
 * @param label - Visible text label for the button
 * @param count - Optional numeric badge shown on the right when greater than zero
 * @param onClick - Click handler invoked when the button is pressed
 * @param active - When true, applies the active visual styling
 * @returns A JSX element representing the styled toolbar button
 */
...
/**
 * Exposes controls to record audio from the user's microphone and deliver the recorded audio as a base64 data URL.
 *
 * @param onDone - Callback invoked when recording stops with the recorded audio data URL and the recording duration in seconds
 * @returns An object with:
 *   - `recording`: `true` while recording is active, `false` otherwise.
 *   - `elapsed`: number of seconds elapsed since the current recording started.
 *   - `start`: begins a new recording session and updates `recording`/`elapsed`.
 *   - `stop`: stops the current recording and triggers `onDone` with the final data.
 */
...
// MAIN PAGE
/**
 * Render the New Entry page: a client-side editor for composing a text entry with modular blocks (images, voice notes, doodles, goals, and task lists) that are persisted locally and synchronized to the backend.
 *
 * @returns The React element for the New Entry editor, including the persisted text area, block grid, toolbar, block-creation modals, localStorage save status indicator, and debounced DB sync behavior.
 */
